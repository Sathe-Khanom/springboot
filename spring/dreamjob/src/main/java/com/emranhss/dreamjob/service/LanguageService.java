...
public class LanguageService {
    ...
    public Language updateLanguage(Long id, Language updatedLanguage, String email) {
        ...
        Language existingLanguage = languageRepository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Language record not found with id: " + id));

        // Optional: Ensure the education belongs to the logged-in job seeker
        if (!existingLanguage.getJobSeeker().getId().equals(jobSeeker.getId())) {
            throw new SecurityException("You are not authorized to update this Language record.");
        }
